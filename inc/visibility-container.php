<?php
/**
 * Visibility container block: role / login-state rules for inner content.
 *
 * @package clanspress
 */
...
function clanspress_visibility_container_should_show( array $attributes, $block = null ): bool {
	...
	$show_roles = isset( $attributes['showToRoles'] ) && is_array( $attributes['showToRoles'] )
		? clanspress_visibility_container_sanitize_role_slugs( $attributes['showToRoles'] )
		: array();
	$hide_roles = isset( $attributes['hideFromRoles'] ) && is_array( $attributes['hideFromRoles'] )
		? clanspress_visibility_container_sanitize_role_slugs( $attributes['hideFromRoles'] )
		: array();
	...
	switch ( $show_to ) {
		...
		case 'logged_in':
			...
			break;
		case 'roles':
			$show_match = $logged_in && clanspress_visibility_container_user_has_any_role( $show_roles, $user_roles );
			if ( empty( $show_roles ) ) {
				$show_match = false;
			}
			break;
	...
	switch ( $hide_from ) {
		...
		case 'logged_in':
			...
			break;
		case 'roles':
			$hide_match = $logged_in && clanspress_visibility_container_user_has_any_role( $hide_roles, $user_roles );
			if ( empty( $hide_roles ) ) {
				$hide_match = false;
			}
			break;
