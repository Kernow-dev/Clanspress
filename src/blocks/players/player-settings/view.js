...
const { state, actions } = store( 'clanspress-player-settings', {
	state: {
		...
		isNavExpanded() {
			...
			if ( state.root ) {
				const element = state.root.querySelector( `#${ attributes.id }` );
				if ( element ) {
		...
		isThisPanelActive() {
			const { attributes } = getElement();
			return this.activePanel === parseArg( attributes[ 'data-wp-args' ] );
		},
	...
	actions: {
		toggleNav() {
			...
			const navId = parseArg( ref?.attributes?.[ 'data-wp-args' ]?.value ?? '' );
			...
			state.activeNav =
				state.activeNav === navId ? null : navId;
		},

		showPanel() {
			const { ref } = getElement();
			const panelId = parseArg( ref?.attributes?.[ 'data-wp-args' ]?.value ?? '' );
		...
		showToast({type = 'success', heading = '', message = '', duration = 6000}) {
		...
		async save() {
			...
			const avatarInput = state.root.querySelector('input[name="profile_avatar"]');
			const coverInput = state.root.querySelector('input[name="profile_cover"]');
	...
	callbacks: {
		init() {
			...
			const navButton = firstNav.querySelector(
				'.nav-item-header'
			);
			const subItem = firstNav.querySelector(
				'.nav-sub-item'
			);
