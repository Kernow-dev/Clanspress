...
const { state, actions } = store( STORE_NAMESPACE, {
	...
	actions: {
		togglePanel: createClanspressToolbarPanelToggler( getTeamAvatarState, {
			...
			islandRootSelector: CLANSPRESS_MEDIA_ISLAND_ROOT_SELECTORS.teamAvatar,
		} ),
		...
		selectFile() {
			...
			const root = getClanspressIslandRootFromRef(
				ref,
				CLANSPRESS_MEDIA_ISLAND_ROOT_SELECTORS.teamAvatar
			);
			root?.querySelector( 'input[name="team_avatar"]' )?.click();
		},
